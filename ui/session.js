...
(function(window) {
    ...
    class SessionManager {
        ...
        save() {
            try {
                if (this.sessionKey && this.user) {
                    ...
                    localStorage.setItem(this.config.storageKeyUser, JSON.stringify(this.user));
                }
            } catch (error) {
        ...
        async refreshSession() {
            ...
            try {
                const response = await fetch(`${this.config.apiEndpoint}?q=session.refresh&ses=${this.sessionKey}`);
        ...
        startHeartbeat() {
            ...
            this.heartbeatInterval = setInterval(() => {
                if (this.isLoggedIn()) this.sendHeartbeat();
            }, this.config.heartbeatIntervalMs);
        }

        async sendHeartbeat() {
            try {
                const response = await fetch(`${this.config.apiEndpoint}?q=session.heartbeat&ses=${this.sessionKey}`);
                const data = await response.json();
        ...
        async login(username, password) {
            try {
                const response = await fetch(`${this.config.apiEndpoint}?q=login.${encodeURIComponent(username)}.${encodeURIComponent(password)}`);
                const data = await response.json();
                if (data.success && data.session) {
        ...
        async signup(username, password, role = 'user') {
            try {
                const response = await fetch(`${this.config.apiEndpoint}?q=signup.${encodeURIComponent(username)}.${encodeURIComponent(password)}${role !== 'user' ? '.' + role : ''}`);
                const data = await response.json();
                if (data.success && data.session) {
                    this.setSession(data.session, data.user);
                    this.emitEvent('signup', { user: data.user });
                    return { success: true, user: data.user };
                }
        ...
        async logout() {
            if (this.isLoggedIn()) {
                try {
                    await fetch(`${this.config.apiEndpoint}?q=logout&ses=${this.sessionKey}`);
                } catch(e) {}
            }
            this.clear();
        ...
        async changePassword(oldPassword, newPassword) {
            ...
            try {
                const response = await fetch(`${this.config.apiEndpoint}?q=user.changepass.${encodeURIComponent(oldPassword)}.${encodeURIComponent(newPassword)}&ses=${this.sessionKey}`);
                const data = await response.json();
                if (data.success) {
                    this.emitEvent('passwordChanged', {});
                }
                return data;
            } catch (error) {
                return { success: false, error: error.message };
        ...
        async getCurrentUser() {
            ...
            try {
                const response = await fetch(`${this.config.apiEndpoint}?q=user.info&ses=${this.sessionKey}`);
                const data = await response.json();
                if (data.success) {
                    this.user = data.user;
                    this.save();
        ...
        updateUI() {
            ...
            if (this.isLoggedIn() && this.user) {
                ...
                if (userRole) userRole.textContent = this.role || 'user';
        ...
        async validateSession() {
            if (!this.isLoggedIn()) return false;
            try {
                const response = await fetch(`${this.config.apiEndpoint}?q=session.validate&ses=${this.sessionKey}`);
                const data = await response.json();
    ...
    document.addEventListener('DOMContentLoaded', () => {
        const protectedPages = ['/dashboard', '/studio', '/studio/', '/cli', '/cli/', '/db', '/settings', '/admin'];
        const currentPath = window.location.pathname;
        const isLoginPage = currentPath === '/login' || currentPath === '/auth.html';
        const isErrorPage = currentPath === '/errors.html';
        
        if (!isLoginPage && !isErrorPage && protectedPages.some(p => currentPath.startsWith(p))) {
            if (!sessionManager.isLoggedIn()) {
                window.location.href = '/login';
            }
        }
        
        if (isLoginPage && sessionManager.isLoggedIn()) {
